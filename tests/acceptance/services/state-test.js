...
module('Integration | Service | state', (hooks) => {
  ...
  test('it returns current state', async function (assert) {
    ...
    assert.equal(
      this.service.get('current.index'),
      pointer + 1,
  ...
  test('it returns current and last state on back', async function (assert) {
    ...
    assert.equal(
      this.service.get('next.index'),
      pointer + 2,
      'next index points to history after current index'
    );
    assert.equal(
      this.service.get('current.index'),
      pointer + 1,
  ...
  test('it returns ordered navigation states', async function (assert) {
    ...
    assert.equal(
      states[1],
      states.findBy('index', pointer),
      'first state exists in first position'
    );
    assert.equal(
      states[2],
      states.findBy('index', pointer + 1),
      'first state exists in first position'
    );
    assert.equal(
      states[3],
      states.findBy('index', pointer + 2),
      'first state exists in first position'
    );
    assert.equal(
      states[4],
      states.findBy('index', pointer + 3),
      'first state exists in first position'
    );
  });
  ...
  skip('it has no previous pointer when replacing first route', async function (assert) {
    ...
    assert.notOk(
      this.service.get('previous'),
      'previous index points to nowhere'
    );
    assert.equal(
      this.service.get('last.index'),
      pointer + 0,
